...
let game = {
    ...
    init() {
        ...
        // устанавливаем размер и цвет шрифта тут, а не в рендере, что б не было обновлений каждый кадр
        this.setTextFont();
        this.setEvents();
    },
    setTextFont() {
    ...
    // устанавливаем обработку событий ввода клавиатуры
    setEvents() {
    ...
    // создаем и подгружаем изображения
    preload(callback) {
        let loaded = 0;
    ...
    // рандомайзер для стартового направления мяча
    random(min, max) {
...
game.platform = {
    ...
    // запрет пересечения границ мира
    collideWorldBounds() {
        let x = this.x + this.dx;
